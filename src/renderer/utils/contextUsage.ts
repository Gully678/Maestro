...
export function estimateAccumulatedGrowth(
	...
	const prevTokens = Math.round((currentUsage / 100) * contextWindow);
	const estCalls = Math.max(1, Math.round((cacheReadTokens || 0) / Math.max(prevTokens, 1)));
	...
	// Bound to 1-3% per turn (conservative to avoid overshooting)
	const boundedGrowth = Math.max(1, Math.min(growthPercent, 3));
